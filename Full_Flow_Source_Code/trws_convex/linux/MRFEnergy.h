...
public:
	...
	struct Options
	{
		Options()
		{
			...
			m_eps = -1; // not used
			m_iterMax = 1000000;
			m_printIter = 1;     // After 10 iterations start printing the lower bound
			m_printMinIter = 1; // and the energy every 5 iterations.
		}
	...
	int Minimize_TRW_S(Options& options, double & lowerBound, double & energy, REAL* min_marginals = NULL);
...
private:
	...
	double ComputeSolutionAndEnergy(); // sets Node::m_solution, returns value of the energy
	REAL ComputeSolutionAndEnergy(int firstLabel); // cqf
...
{
	//return i->m_solution;
	return i->best_solution;//cqf
}

using namespace std;
template <class T> int MRFEnergy<T>::Minimize_TRW_S(Options& options, double & mylowerBound, double & energy, REAL* min_marginals)
{

	int iter;
	double lowerBoundPrev,lowerBound;
	...
	int flag[m_nodeNum];
	vector<Node*> all[m_nodeNum];
	memset(flag,0,sizeof(flag));
	for (Node *i=m_nodeFirst; i; i=i->m_next){
		for (MRFEdge *e=i->m_firstForward; e; e=e->m_nextForward){
			Node *j = e->m_head;
			if(flag[j->m_ordering]==0)
				flag[j->m_ordering]=flag[i->m_ordering]+1;
		}
		all[flag[i->m_ordering]].push_back(i);
	}
	...
	for (iter=1; ; iter++)
	{
		...
		for(int depth=0;depth<m_nodeNum;++depth)
		if(all[depth].size()>0)
		#pragma omp parallel for num_threads(12)
		for(int id=0;id<all[depth].size();++id)
		{
			Node* i;
			Node* j;
			REAL vMin;
			MRFEdge* e;		
			i=all[depth][id];
			char mybuf[m_vectorMaxSizeInBytes + 		( m_vectorMaxSizeInBytes > Edge::GetBufSizeInBytes(m_vectorMaxSizeInBytes) ?		  m_vectorMaxSizeInBytes : Edge::GetBufSizeInBytes(m_vectorMaxSizeInBytes) )];
			Vector* Di = (Vector*) mybuf;
			void* buf = (void*) (mybuf + m_vectorMaxSizeInBytes);			
		...
		for(int depth=m_nodeNum-1;depth>=0;--depth)
		if(all[depth].size()>0)
		#pragma omp parallel for reduction(+:lowerBound) num_threads(12)
		for(int id=0;id<all[depth].size();++id)		
		{
			Node* i;
			Node* j;
		...
		)
		{
			...
			if(energy<bestEnergy){//cqf
				for (Node *i=m_nodeFirst; i; i=i->m_next)
					i->best_solution=i->m_solution;
				bestEnergy=energy;
			}
/*			if(iter>1&&energy==bestEnergy)
				for(int firstLabel=0;firstLabel<Di->GetArraySize(m_Kglobal, m_nodeFirst->m_K);firstLabel++){
					energy = ComputeSolutionAndEnergy(firstLabel);
					mexPrintf("%f\n",energy);
					mexEvalString("drawnow;");
...
template <class T> double MRFEnergy<T>::ComputeSolutionAndEnergy()
{
	...
	for(int id=0;id<all[depth].size();++id)
	{
		...
		Node* j;
		MRFEdge* e;
		char buffer[m_vectorMaxSizeInBytes*2];
		Vector* DiBackward = (Vector*) buffer; // cost of backward edges plus Di at the node
		Vector* Di = (Vector*) (buffer + m_vectorMaxSizeInBytes); // all edges plus Di at the node
...
//cqf
template <class T> typename T::REAL MRFEnergy<T>::ComputeSolutionAndEnergy(int firstLabel)
{
	Node* i;
	Node* j;
	MRFEdge* e;
	...
	for (i=m_nodeFirst; i; i=i->m_next)
	{
		...
		if(i==m_nodeFirst)//cqf
			i->m_solution=firstLabel;
		else
			Di->ComputeMin(m_Kglobal, i->m_K, i->m_solution);
